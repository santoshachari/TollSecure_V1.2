...
public class TollTransactionDAOImpl implements TollTransactionDAO {
	...
	public List<Object[]> getTransactionsOfAVehicle(String laneId, String startTime, String endTime, String date, String vnumber, String vehicleClass) {
		...
		theQuery = currentSession.createQuery("select tt.transactionId, tt.vehicleNumber, tt.ticketCode, tc.vehicleClass, tt.journeyType, tt.tollAmt, tt.createTimeStamp, "
				+ " tt.shiftDescription, tt.imageAddress, tu.userFirstName, tu.userLastName, COALESCE(tp.passType, '0'), COALESCE(tt.exemptId, '0'), COALESCE(tt.concessionId, '0') from TollTransaction tt join TollConfig tc on tt.vehicleClassId = tc.vehicleClassId"
				+ " join User tu on tu.userId = tt.createUserID left join Pass tp on (tt.passId is not null and tt.passId = tp.passId) "
	...
	public List<Object[]> getImageTransaction(String transactionId) {
		...
		Query<Object[]> theQuery = currentSession.createQuery("select tt.ticketCode, tt.transactionTimeStamp,"
				...
				+ "tc.vehicleClass, tt.journeyType,  COALESCE(tp.passType, '0'), COALESCE(tt.exemptId, '0'), COALESCE(tt.concessionId, '0') from User u join TollTransaction tt on tt.createUserID = u.userId join TollConfig tc"
				+ " on tc.vehicleClassId = tt.vehicleClassId left join Pass tp on (tt.passId is not null and tp.passId = tt.passId) "
